...
| :------------: | :---: | :--- | :--- | :------: | :---: |
| 第一周  11.4-11.10 | 基础数据结构 | 队列，栈，DFS，BFS | 熟悉C++ STL的queue, deque, stack |  |  |
| 第二周  11.11-11.17 | 基础排序，搜索 | 归并排序，二分查找  |  |  |  |
| 第三周  11.18-11.24 | 基础排序，数据结构 | 链表，快排（partition算法）  | 熟悉C++的指针用法 |  |  |
| 第四周  11.25-12.1 | 数据结构深入 | 单调栈，单调队列，优先队列，并查集  |  |  |  |
| 第五周  12.2-12.8 | 数论入门 | GCD & LCM，裴蜀定理  |  |  |  |
| 第六周  12.9-12.15 | 数论入门 | 素数筛法（欧拉筛），分解质因数，大数取模，快速幂  |  |  |  |
| 第七周  12.16-12.22 | 动态规划入门 | 背包问题（0/1背包，完全背包），最大子数组和，最长上升子序列，找钱问题 |  |  |  |
| 第八周  12.23-12.29 | 树，图入门 | 最短路径算法（Dijkstra & Floyd），树和图的搜索查找（DFS/BFS） | 学会C++建图 & 树  |  |  |
| 第九周  12.30-1.5 | 图论算法加强 | 拓扑排序，树上倍增（LCA），树上DP | 深入理解凸轮 |  |  |
| 第十周  1.6-1.12 | 字符串 | Trie，KMP算法，Rabin-Karp算法 |  |  |  |
| 第十一周 1.13-1.19 | 位运算操作 | 各种位运算(与, 或, 异或,非, lowbit)性质 |  |  |  |
| 第十二周 1.20-1.26 | 博弈入门 | Nim游戏，SG函数，Grundy numbers（Mex） |  |  |  |
|  |  |  |  |  |  |
