# TUST-ACM战队简述
TUST-ACM战队成立于2024年5月，初始成员由贾舒奥维，赵子涵，王森，韩高臻组成。TUST-ACM战队的目标是代表TUST参与xcpc（icpc/ccpc）比赛。
# TUST-ACM战队成员
# todo 大家把自己的github id填写一下
| 姓名/id | 年级 | 工作 |
|----------|----------|----------|
| guaidaokakaxi    | 18     | Data     |
| Row 2    | Data     | Data     |
| Row 3    | Data     | Data     |
| Row 4    | Data     | Data     |
| Row 5    | Data     | Data     |
| Row 6    | Data     | Data     |
| Row 7    | Data     | Data     |
| Row 8    | Data     | Data     |
| Row 9    | Data     | Data     |
| Row 10   | Data     | Data     |
| Row 11   | Data     | Data     |
| Row 12   | Data     | Data     |
| Row 13   | Data     | Data     |
| Row 14   | Data     | Data     |
| Row 15   | Data     | Data     |
| Row 16   | Data     | Data     |
| Row 17   | Data     | Data     |
| Row 18   | Data     | Data     |
| Row 19   | Data     | Data     |
| Row 20   | Data     | Data     |
| Row 21   | Data     | Data     |
| Row 22   | Data     | Data     |
| Row 23   | Data     | Data     |
| Row 24   | Data     | Data     |
| Row 25   | Data     | Data     |
| Row 26   | Data     | Data     |
| Row 27   | Data     | Data     |
| Row 28   | Data     | Data     |

# TUST-ACM战队 算法入队选拔机制
因为经费有限&&xcpc本身有名额限制，所以不会所有人都去打xcpc（蓝桥杯等没有名额限制的竞赛不再讨论范围内），所以我们会有内部的算法入队选拔赛：

11-1月份是队内集训，集训后我们会有8-12场左右的线上竞赛，这些线上竞赛可能是codeforces，atcoder，力扣，牛客竞赛，蓝桥竞赛等线上比赛；每场比赛我们会统计参赛队员的得分，得分规则如下：

| 比赛名次 | 得分 | 备注 |
|----------|----------|----------|
| 1    | 20     | 无     |
| 2    | 15     | 无     |
| 3    | 12     | 无     |
| 4    | 10     | 无     |
| 5    | 8     | 无     |
| 6    | 7     | 无     |
| 7    | 6     | 无     |
| 8    | 5     | 无     |
| 9    | 4     | 无     |
| 10    | 3     | 无     |
| >10    | 1     | 无     |
| 未参加比赛    | 0     | 无     |

那么，8-12场比赛后，每个人都会有一个自己的得分；最终我们会产出一个队内每个人的得分，得分作用是建队，具体建队规则如下：
1. 总得分第一名优先挑选自己的两名队友并和这两人协商，如果这两人同意组队（例如第一名挑选了第3名和第5名作为队友&&第三名和第五名均同意），那么这个队伍就会作为TUST的算法一队。
2. 接着，看下一个没有成立队伍的最高名次，此时是第二名，那么，第二名优先选择自己的两位队友并征求两名队友的同意后成立TUST的算法二队
3. 第一届算法队伍不超过4个队伍（如果有女队，那么女队另算）
4. 剩下没有入队的同学会分流去张老师的项目组进行项目学习和参与；但是并非这类同学就没有机会回来打xcpc，未来这类同学如果要回来打xcpc有两个途径，第一cf/atc分达到TUST二队的第一名的水平，第二如果经费充足那么可能可以参加ccpc的省赛
5. 以上规则仅适用xcpc


# TUST-ACM战队 训练计划
## 概述
11-1月训练计划
预估的每周平均学习时间得在30-50小时左右才能完成
## 运行机制
每周都会有一个小组作为负责人，负责人由3人一组组成，一个人最多当2次负责人；大家之间互相服务
### 负责人工作内容
#### 提前学习负责该周的内容
#### 选题单
每周作业题单题量在10-20题（取决于题目难度，如果偏难题目数量就少一点，不要脱离大部队的实力选题，比如选cf2000分的题，没有任何意义），题单选择可参考下面：
https://github.com/EndlessCheng/codeforces-go/blob/master/leetcode/SOLUTIONS.md
https://github.com/Lightblues/Leetcode
题单不用局限于什么平台，力扣，cf，atc等等都无所谓，关键是保证题目质量
#### 3人互相验题
举个例子，比如hangz,zzh,ws三人一组是第12周的负责人，那么至少提前一周，也就是第11周要准备好第12周的题目，并且3人之间都把选出来的12-20题全部做完，保证题目质量
#### 3人负责关注本周自己选的题单的群内完成情况
需要参考
https://github.com/users/guaidaokakaxi/projects/3/views/2
模仿第一周创建一个第N周的表格（直接复制第一周）然后改一下列名为当前选的题目
#### 所有成员需要在github的projects上更新自己的题目完成情况 类似这样:
https://github.com/users/guaidaokakaxi/projects/3/views/2

#### 实验室负责人和本周3为负责人的工作
第x周的3位负责人需要在第x-1周周四准备好题单，并把题单发给hangz，zzh，ws三位实验室负责人过目，没有问题之后，第x周的3位负责人需要在第x-1周的周日前准备好验题，验题后把本周题单链接（github地址）发到群里并at所有同学通知他们下周的题目；

实验室负责人hangz，zzh，ws 负责关注每一组负责人的完成进度，比如12周的题单，需要在11周的周四前选出，需要在11周的周日前完成验题

| 时间               | 主题                                             | 可选主题                          | 负责小队 |
|--------------------|--------------------------------------------------|-----------------------------------|----------|
| 第1周（基础数据结构） | 队列，栈，DFS，BFS                             | 熟悉C++ STL的queue, deque, stack |          |
| 第2周（基础排序，搜索） | 归并排序，二分查找                             |                                   |          |
| 第3周（基础排序，数据结构） | 链表，快排（partition算法）                   | 熟悉C++的指针用法                |          |
| 第4周（数据结构深入） | 单调栈，单调队列，优先队列，并查集             |                                   |          |
| 第5周（数论入门）     | GCD & LCM，裴蜀定理                            |                                   |          |
| 第6周（数论入门）     | 素数筛法（欧拉筛），分解质因数，大数取模，快速幂 |                                   |          |
| 第7周（动态规划入门） | 背包问题（0/1背包，完全背包），最大子数组和，最长上升子序列，找钱问题 |                                   |          |
| 第8周（树，图入门）   | 最短路径算法（Dijkstra & Floyd），树和图的搜索查找（DFS/BFS） | 学会C++建图 & 树                   |          |
| 第9周（图论算法加强） | 拓扑排序，树上倍增（LCA），树上DP              | 深入理解凸轮                      |          |
| 第10周（字符串）      | Trie，KMP算法，Rabin-Karp算法                  |                                   |          |
| 第11周（位运算操作）  | 各种位运算(与, 或, 异或,非, lowbit)性质               |                                   |          |
| 第12周（博弈入门）    | Nim游戏，SG函数，Grundy numbers（Mex）         |                                   |          |

后续还有继续深入的训练，这里暂列11-1月

# 每周题单（每周负责人更新这里，完全模仿第一周格式）
## 第一周（11月4号-11月10号）
| 知识点       | 题目                                                                                                              | 题解                                                   | 难度 |
|--------------|------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|------|
| stack        | [有效的括号](https://leetcode.cn/problems/valid-parentheses/description/?envType=problem-list-v2&envId=stack)     | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 简单 |
| stack        | [最小栈](https://leetcode.cn/problems/min-stack/description/?envType=problem-list-v2&envId=stack)                 | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 简单 |
| stack        | [用栈实现队列](https://leetcode.cn/problems/implement-queue-using-stacks/description/?envType=problem-list-v2&envId=stack) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 简单 |
| stack，queue | [用队列实现栈](https://leetcode.cn/problems/implement-stack-using-queues/description/?envType=problem-list-v2&envId=queue) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 简单 |
| queue        | [设计循环队列](https://leetcode.cn/problems/design-circular-queue/description/?envType=problem-list-v2&envId=queue) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 中等 |
| queue        | [Dota2 参议院](https://leetcode.cn/problems/dota2-senate/description/?envType=problem-list-v2&envId=queue)       | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 中等 |
| dfs          | [二叉树的中序遍历](https://leetcode.cn/problems/binary-tree-inorder-traversal/description/?envType=problem-list-v2&envId=depth-first-search) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 简单 |
| 搜索         | [岛屿数量](https://leetcode.cn/problems/number-of-islands/description/?envType=problem-list-v2&envId=breadth-first-search) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 中等 |
| stack        | [最长有效括号](https://leetcode.cn/problems/longest-valid-parentheses/description/?envType=problem-list-v2&envId=stack) | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 困难 |
| stack        | [接雨水](https://leetcode.cn/problems/trapping-rain-water/description/?envType=problem-list-v2&envId=stack)      | 力扣题解区找高赞答案（可优先看灵茶山艾府，liweiwei1419，宫水三叶） | 困难 |




# 时间轴timeline
1. 11-1月三个月，集中训练，夯实基础
2. 2月-3月，寒假集训&队内赛，统一在实验室参加8-12场线上竞赛，每个人取得个人积分
3. 根据个人积分分出1～4队，没进队同学分流进项目组（没进队可以自己继续训练算法）
4. 4月蓝桥杯，全员参与，后续所有算法竞赛如果要填指导老师都要填上张传雷教授
5. 5月-6月ccpc省赛，暂定至少参加河北河南两场省赛
6. 7-8月暑假集训，准备icpc
7. 9月开学的前三周(一般这样)，每个周末一把网络预选赛，争取icpc学校的参赛资格，要求全员参与
8. 10月-12月，icpc正式举办的日子，icpc 三人vp&正式参赛
